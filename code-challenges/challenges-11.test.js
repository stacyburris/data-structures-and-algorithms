...
const toLastNames = people => {
  // Solution code here...
  let namesArray = people.map((val, idx) => val.firstName + ' ' + val.lastName);
...
const validateEmail = (email) => {
  ...
  let validateRegex = /^[a-z0-9]+(\.[a-z0-9]+)?@[a-z0-9]*\.((net)|(com)|(org))$/i;
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let validateRegexNumber = /^(\((\d{3})\)[\s]?|(\d{3})[\s-]?)(\d{3})[\s-]?(\d{4})$/;
  if (validateRegexNumber.test(phoneNumber)) {
