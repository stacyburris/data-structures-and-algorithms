...
const totalSum = (input) => {
  ...
  let newArray = input.flat();
  let newSumArray = newArray.reduce((acc, val, idx) => {
    acc = acc += val;
    //acc.push(newSumArray);
    //console.log(acc);
    return acc;
  }, 0);
  return newSumArray;
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let betterArray = input.map(numbers => {
    return numbers.filter(betterArray => (!(betterArray % 5) && typeof betterArray === 'number'));
  })
  let doubleArray = [];
  numbers.forEach(raised => {

  })
  let betterArrayTwo = betterArray.map(raised => Math.pow(2, raised));
  return betterArrayTwo;
}



// let betterArray = input.reduce((acc, val, idx) => {
//   let newFilter = acc.filter(number => {
//     if betterArray = 0 && val % 5 === 0)
//       console.log(acc);
//     return acc;
