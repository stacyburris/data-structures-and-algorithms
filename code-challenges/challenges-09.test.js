...
const mapCurrentEvents = () => {
  ...
  let currentEventsArray = currentEvents.news.map(obj => new Event(obj));
  return currentEventsArray;
  //response.send(currentEvents);
}
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let newElement = arr.reduce((acc, val, idx) => {
    acc = idx + 1;
    //console.log(acc);
    return acc;
...
const returnNames = (arr) => {
  // Solution code here...
  let starWars = arr.reduce((acc, val, idx) => {
    acc.push(val.name);
    console.log(acc);
...
const reversedString = (str) => {
  ...
  let reverseLetters = arr.reduce((newString, currentLetter) => {
