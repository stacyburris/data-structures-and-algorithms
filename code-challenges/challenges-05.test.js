...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(element => {
    ...
    newTemplate.find('h2').text(element.name);
    newTemplate.find('h3').text(element.height);
    newTemplate.find('p').text(element.eye_color);
    newTemplate.removeAttr('id');
    $('main').append(newTemplate);
  })
  return templateWithJQuery;
...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i <= str.length; i++) {
    let newPencil = str.slice(i);
    result.push(newPencil);
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(data => {
    let foodName = data.indexOf(' ');
    let actualFood = data.indexOf(' ', foodName + 1);
    result.push(data.slice(actualFood + 1, data.length));
  })
