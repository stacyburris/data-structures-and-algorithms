...
/* 
 // Solution code here...
  ...
  //   let newArray = [];
  //   arr.map((data) => {
  //     newArray.push(data[0].toUpperCase() + data.substring(1));
  //   })
  //   return newArray;
  // };
------------------------------------------------------------------------------------------------
...
let biggerThanLuke = (arr) => {
  ...
  let massName = arr.reduce((acc, person) => {
    if (parseInt(person.mass) > parseInt(arr[0].mass)) {
      acc += `${person.name} - `
    };
    console.log(acc);
    return acc;
  ...
  return massName.slice(0, massName.length - 3);
...
const sortBy = (property, arr) => {
  // Solution code here...
  arr.sort((n, p) => {
    console.log('why won\'t you pass', n, p);
    return n[property] - p[property];
